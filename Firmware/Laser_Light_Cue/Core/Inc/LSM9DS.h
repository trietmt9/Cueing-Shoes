...
typedef struct
{
    uint8_t enabled;
    XL_Scale_t scale;
    Accel_OP_DataRate_t sampleRate;
    Accel_abw bandwidth;
}XL_settings;
...
uint8_t LSMD9S_Init(SPI_TypeDef* SPI_x);
float LSMD9S_Read_XLG(SPI_TypeDef* SPI_x);
float LSMD9S_Read_Mag(SPI_TypeDef* SPI_x);
#endif
